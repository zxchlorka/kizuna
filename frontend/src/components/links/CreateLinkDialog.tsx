...
import type { LinkKind, RedisExtract } from '@/types/api'

interface CreateLinkDialogProps {
  open: boolean
  sourceConnId: string
  sourceKind: LinkKind
  sourceScope: string // kafka: topic; postgres: table; redis: suggested key pattern (editable)
  sourceFieldOptions?: string[] // kafka: json keys; postgres: columns; redis: hash field names
...
export function CreateLinkDialog({
  ...
  const [scopeInput, setScopeInput] = useState(sourceScope)
  ...
  const needsSourceField =
    sourceKind === 'kafka' || sourceKind === 'postgres' || (sourceKind === 'redis' && redisExtract === 'value_field')
  ...
  const effectiveScope = sourceKind === 'redis' ? scopeInput.trim() : sourceScope
  ...
  const invalid =
    ...
    (sourceKind === 'redis' && redisExtract === 'key_capture' && (effectiveScope.match(/\*/g)?.length ?? 0) !== 1) ||
  ...
  const handleSave = async () => {
    ...
    try {
      await createLink({
        ...
        source_extract: sourceKind === 'redis' ? redisExtract : undefined,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Create link from {sourceKind} ({sourceScope})</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          {sourceKind === 'redis' && (
            <>
              <div className="space-y-1">
                <label className={labelClass}>Key pattern (scope)</label>
                <input value={scopeInput} onChange={(event) => setScopeInput(event.target.value)} placeholder="profile:*" className={inputClass} />
              </div>
              <div className="space-y-1">
          ...
          {needsSourceField && (
            <div className="space-y-1">
              <label className={labelClass}>{sourceKind === 'postgres' ? 'Source column' : 'Source field'}</label>
              {fieldControl(sourceField, setSourceField, sourceKind === 'postgres' ? 'Pick a column' : 'Field name / JSON path')}
            </div>
          )}
