...
Веб-приложение для просмотра и управления данными в PostgreSQL (в будущем — Redis и Kafka).
Один инструмент вместо pgAdmin + Redis Desktop Manager + RedPanda Console.

## Что умеет сейчас (Phase 1, Sprint 1)
...
docker-compose.yml                 — infraview + infraview-debug + postgres
```
...
Ключевой принцип — **единый Connector interface**. API-слой не знает тип источника данных, получает `Connector` из `ConnectionManager` и вызывает методы. Фронтенд знает тип только для рендера нужных UI-компонентов.
...
HTTP Request
    ...
    → Connector (Postgres/Redis/Kafka)
    → Response JSON
```
